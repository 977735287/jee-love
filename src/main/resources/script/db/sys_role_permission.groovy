...
databaseChangeLog(logicalFilePath: 'sys_role_permission.groovy') {
    changeSet(author: 'Sanchar', id: '2018-12-26_sys_role_permission') {
        createTable(tableName: 'sys_role_permission', remarks: '角色权限关联表') {
            column(name: 'id', type: 'bigint(20)', autoIncrement: 'true', remarks: '表ID，主键，供其他表做外键') {
                constraints(nullable: 'false', primaryKey: 'true', primaryKeyName: 'pk_sys_role_permission')
            }
            column(name: 'role_id', type: 'bigint(20)', remarks: '角色ID') {
                constraints(nullable: 'false')
            }
            column(name: 'permission_id', type: 'bigint(20)', remarks: '权限ID') {
                constraints(nullable: 'false')
            }
